...
if __name__ == "__main__":
    ...
    for gi, ((i1, p1), (i2, p2), (i3, p3)) in enumerate(combinations(enumerate(pY_Z), 3)):
        for k in range(sK):
            ...
            Red_Triple[gi, k] = (
                Im_Single[i1, k] + Im_Single[i2, k] + Im_Single[i3, k]
            ) - Im_Triple[gi, k]
    # ---- All-Four Schemas ---- #
    Im_Quad = np.empty(sK)
    ...
    print("-------------------------------------------------")
    print(
        " Y = U     | {:4.2f}  {:4.2f}  {:4.2f} |".format(
            Im_Same.mean(), Im_Same.min(), Im_Same.max()
        )
    )
    print("-------------------------------------------------")
    for s in range(sS):
        print(
            " {:3s}       | {:4.2f}  {:4.2f}  {:4.2f} |".format(
                SCHEMAS[s], Im_Single[s, :].mean(), Im_Single[s, :].min(), Im_Single[s, :].max()
            )
        )
    print("-------------------------------------------------")
    for si, (s1, s2) in enumerate(combinations(SCHEMAS, 2)):
        print(
            " {:6s}    | {:4.2f}  {:4.2f}  {:4.2f} | {:4.2f}  {:4.2f}  {:4.2f}".format(
                "{}+{}".format(s1, s2),
                Im_Pair[si, :].mean(),
                Im_Pair[si, :].min(),
                Im_Pair[si, :].max(),
    ...
    for si, (s1, s2, s3) in enumerate(combinations(SCHEMAS, 3)):
        print(
            " {:9s} | {:4.2f}  {:4.2f}  {:4.2f} | {:4.2f}  {:4.2f}  {:4.2f}".format(
                "{}+{}+{}".format(s1, s2, s3),
                Im_Triple[si, :].mean(),
                Im_Triple[si, :].min(),
                Im_Triple[si, :].max(),
    ...
    print("-------------------------------------------------")
    print(
        " ALL       | {:4.2f}  {:4.2f}  {:4.2f} | {:4.2f}  {:4.2f}  {:4.2f}".format(
            Im_Quad.mean(),
            Im_Quad.min(),
            Im_Quad.max(),
            Red_Quad.mean(),
