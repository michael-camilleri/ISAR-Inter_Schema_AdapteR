...
DEFAULTS = {
    ...
    "Pi": "true",  # Pi mode
    "Folds": "10",
}  # Number of Folds to use (folding is by Segment)
...
if __name__ == "__main__":
    ...
    _arg_parse = argparse.ArgumentParser(
        description="Simulate and Train Annotator models (based on DS and ISAR)"
    )
    ...
    _arg_parse.add_argument(
        "-f",
        "--folds",
        help="Number of splits (folds) to operate on per run. Default {}".format(DEFAULTS["Folds"]),
        default=DEFAULTS["Folds"],
    )
    _arg_parse.add_argument(
        "-s",
        "--schemas",
        help="Schema Probabilities. Default is: {}".format(DEFAULTS["Schemas"]),
    ...
    _arg_parse.add_argument(
        "-p",
        "--pi",
        help="How to simulate Pi: can be {{true}} to use the PI learnt from the MRC "
        ...
        'weight for the Dirichlet from which Pi is sampled. Default is "{}"'.format(DEFAULTS["Pi"]),
        default=DEFAULTS["Pi"],
    )
    _arg_parse.add_argument(
        "-a",
        "--all",
        help="If Set, then store all the data (i.e. including the simulation): note "
        "that this will generate large files. Default is off.",
        dest="save_all",
        action="store_true",
    )
    ...
    if args.output[DS].lower() != "none":
        ...
        pred_wght_DS = np.zeros(
            [run_length, sS + 1], dtype=float
        )  # Array for Predictive Log Probability
    ...
    for run in range(run_offset, run_offset + run_length):
        ...
        sampler = InterSchemaAdapteRIID(
            [sZ, sK, sS], omega, params=(pi, psi), random_state=args.random + run
        )
        ...
        if args.output[ISAR].lower() != "none":
            ...
            for fold in range(sF):  # Iterate over folds
                ...
                isar_model = InterSchemaAdapteRIID(
                    [sZ, sK, sS], omega, random_state=args.random + run, sink=sys.stdout
                )
            ...
            for s in range(sS):
                ...
                pred_f1_ISAR[run, s] = f1_score(
                    Z[schema_idcs],
                    z_predictions[schema_idcs],
                    labels=np.arange(sZ),
                    average="macro",
                )
            # And Globals
            ...
            pred_f1_ISAR[run, -1] = f1_score(
                Z, z_predictions, labels=np.arange(sZ), average="macro"
            )
        ...
        if args.output[DS].lower() != "none":
            ...
            for fold in range(sF):  # Iterate over folds
                ...
                priors = [
                    np.ones(sZ) * 2,
                    np.ones([sZ, sK, sU]) * 2,
                ]  # Prior Probabilities (actual alphas)
                starts = [
                    (
                        npext.sum_to_one(np.ones(sZ)),  # Starting Probabilities
                        np.tile(
                            npext.sum_to_one(
                                np.eye(sZ, sU) + np.full([sZ, sU], fill_value=0.01), axis=1
                            )[:, np.newaxis, :],
                ...
                ds_model = DawidSkeneIID(
                    [sZ, sK], None, random_state=args.random + run, sink=sys.stdout
                )
    ...
    if args.output[DS].lower() != "none":
        np.savez_compressed(
            args.output[DS], accuracy=pred_acc_DS, f1=pred_f1_DS, log_loss=-pred_wght_DS
        )
    if args.output[ISAR].lower() != "none":
        np.savez_compressed(
            args.output[ISAR], accuracy=pred_acc_ISAR, f1=pred_f1_ISAR, log_loss=-pred_wght_ISAR
        )
