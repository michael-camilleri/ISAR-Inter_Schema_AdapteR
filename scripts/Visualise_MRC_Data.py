...
DEFAULTS = {'Results': ['../data/Learn_DS.npz', '../data/Learn_ISAR.npz'],
            'Schemas': ['I', 'II', 'III', 'IV']}
...
if __name__ == '__main__':
    ...
    for s in range(len(args.schemas)):
        print(' Schema {:3} | {:.2f} +/- {:4.2f} | {:.2f} +/- {:4.2f} | t={:8.2f}, p={:.2e}'
              .format(args.schemas[s], np.nanmean(ell_ds[:, s]), np.nanstd(ell_ds[:, s]), np.nanmean(ell_isar[:, s]),
                      np.nanstd(ell_isar[:, s]),
                      *ttest_rel(ell_ds[~np.isnan(ell_ds[:, s]), s], ell_isar[~np.isnan(ell_isar[:, s]), s])))
    # [B] - Now do global
    ell_ds_global = np.nansum(ell_ds, axis=-1)
    ell_isar_global = np.nansum(ell_isar, axis=-1)
    print('----------------------------------------------------------------------')
    print(' Global     | {:.2f} +/- {:4.2f} | {:.2f} +/- {:.2f} | t={:8.2f}, p={:.2e}'
          .format(ell_ds_global.mean(), ell_ds_global.std(), ell_isar_global.mean(), ell_isar_global.std(),
