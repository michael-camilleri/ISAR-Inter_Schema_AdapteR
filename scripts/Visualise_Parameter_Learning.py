...
if __name__ == "__main__":
    ...
    _arg_parse = argparse.ArgumentParser(
        description="Visualise the results of Training for Parameter Recover (MRC " "Harwell Data)"
    )
    _arg_parse.add_argument(
        "-r",
        "--results",
        help="Result file. Default is: {}".format(DEFAULTS["Results"]),
        default=DEFAULTS["Results"],
    )
    _arg_parse.add_argument(
        "-i",
        "--independent",
        help="If set, then compute the RAD independently per annotator.",
        action="store_true",
    )
    ...
    for run in range(Pi_full.shape[0]):
        for inc in range(Pi_full.shape[1]):
            ...
            rad_full_pi[run, inc] = (
                np.abs(Pi_true[run] - Pi_full[run, inc]) * 100.0 / np.mean(Pi_true[run])
            ).mean()
            rad_isar_pi[run, inc] = (
                np.abs(Pi_true[run] - Pi_isar[run, inc]) * 100.0 / np.mean(Pi_true[run])
            ).mean()
            # Compute now for Psi
            rad_full_psi[run, inc] = (
                np.abs(Psi_true[run] - Psi_full[run, inc]) * 100.0 / np.mean(Psi_true[run])
            ).mean()
            rad_isar_psi[run, inc] = (
                np.abs(Psi_true[run] - Psi_isar[run, inc]) * 100.0 / np.mean(Psi_true[run])
            ).mean()
            # Now Compute for Psi independently if needed
            if args.independent:
                rad_isar_psi_k[run, inc, :] = (
                    np.abs(Psi_true[run] - Psi_isar[run, inc])
                    * 100.0
                    / np.mean(Psi_true[run], axis=(0, 2), keepdims=True)
                ).mean(axis=(0, 2))

    # ==== Print final values for Posterity ==== #
    print(
        "Using Full: Pi={:.2f}% | Psi={:.2f}%".format(
            np.mean(rad_full_pi[:, -1]), np.mean(rad_full_psi[:, -1])
        )
    )
    print(
        "Using ISAR: Pi={:.2f}% | Psi={:.2f}%".format(
            np.mean(rad_isar_pi[:, -1]), np.mean(rad_isar_psi[:, -1])
        )
    )
    ...
    # [A] - Plot
    plt.errorbar(
        Data_sizes.mean(axis=0),
        np.mean(rad_isar_pi, axis=0),
        yerr=np.std(rad_isar_pi, axis=0),
        label="$\Pi$ (ISAR)",
        linewidth=3.0,
        color="b",
        linestyle="-",
    ...
    plt.errorbar(
        Data_sizes.mean(axis=0),
        np.mean(rad_full_pi, axis=0),
        yerr=np.std(rad_full_pi, axis=0),
        label="$\Pi$ (Full)",
        linewidth=3.0,
        color="b",
        linestyle="--",
    ...
    plt.errorbar(
        Data_sizes.mean(axis=0),
        np.mean(rad_isar_psi, axis=0),
        yerr=np.std(rad_isar_psi, axis=0),
        label="$\Psi$ (ISAR)",
        linewidth=3.0,
        color="red",
        linestyle="-",
    ...
    plt.errorbar(
        Data_sizes.mean(axis=0),
        np.mean(rad_full_psi, axis=0),
        yerr=np.std(rad_full_psi, axis=0),
        label="$\Psi$ (Full)",
        linewidth=3.0,
        color="red",
        linestyle="--",
    ...
    if args.independent:
        ...
        for k in range(sK):
            plt.errorbar(
                Data_sizes.mean(axis=0),
                np.mean(rad_isar_psi_k[:, :, k], axis=0),
                yerr=np.std(rad_isar_psi_k[:, :, k], axis=0),
                label="$\Psi$ (A {})".format(k),
                linewidth=3.0,
            )
