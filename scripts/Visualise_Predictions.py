...
DEFAULTS = {
    "Results": ["../data/Compare_DS.npz", "../data/Compare_ISAR.npz"],
    "Schemas": ["I", "II", "III", "IV"],
}
...
if __name__ == "__main__":
    ...
    for s in range(len(args.schemas)):
        print(
            " Schema {:3} | {:.1f}% +/- {:4.2f}% | {:.1f}% +/- {:.2f}% | t={:8.2f}, p={:.2e}".format(
                args.schemas[s],
                accuracy_ds[:, s].mean() * 100,
                accuracy_ds[:, s].std() * 100,
                accuracy_isar[:, s].mean() * 100,
                accuracy_isar[:, s].std() * 100,
    ...
    print("--------------------------------------------------------------------------")
    print(
        " Global     | {:.1f}% +/- {:.2f}% | {:.1f}% +/- {:.2f}% | t={:8.2f}, p={:.2e}".format(
            accuracy_ds[:, -1].mean() * 100,
            accuracy_ds[:, -1].std() * 100,
            accuracy_isar[:, -1].mean() * 100,
            accuracy_isar[:, -1].std() * 100,
    ...
    for s in range(len(args.schemas)):
        print(
            " Schema {:3} | {:6.2f} +/- {:4.2f} | {:.2f} +/- {:.2f}  | t={:8.2f}, p={:.2e}".format(
                args.schemas[s],
                log_loss_ds[:, s].mean(),
                log_loss_ds[:, s].std(),
                log_loss_isar[:, s].mean(),
                log_loss_isar[:, s].std(),
    ...
    print("--------------------------------------------------------------------------")
    print(
        " Global     |  {:.2f} +/- {:.2f} | {:5.2f} +/- {:4.2f}  | t={:8.2f}, p={:.2e} ".format(
            log_loss_ds[:, -1].mean(),
            log_loss_ds[:, -1].std(),
            log_loss_isar[:, -1].mean(),
            log_loss_isar[:, -1].std(),
    ...
    for s in range(len(args.schemas)):
        print(
            " Schema {:3} | {:.3f} +/- {:4.3f} | {:.3f} +/- {:.3f} | t={:8.2f}, p={:.2e}".format(
                args.schemas[s],
                f1_score_ds[:, s].mean(),
                f1_score_ds[:, s].std(),
                f1_score_isar[:, s].mean(),
                f1_score_isar[:, s].std(),
    ...
    print("--------------------------------------------------------------------------")
    print(
        " Global     | {:.3f} +/- {:.3f} | {:.3f} +/- {:.3f} | t={:8.2f}, p={:.2e}".format(
            f1_score_ds[:, -1].mean(),
            f1_score_ds[:, -1].std(),
            f1_score_isar[:, -1].mean(),
            f1_score_isar[:, -1].std(),
