...
DEFAULTS = {'Results': ['../data/Compare_DS.npz', '../data/Compare_ISAR.npz', '../data/Compare_True.npz'],
            'Schemas': ['I', 'II', 'III', 'IV']}
...
if __name__ == '__main__':
    ...
    _arg_parse.add_argument('-r', '--results', help='List of (3) Result files for the DS/ISAR/True models respectively '
    ...
    with np.load(args.results[2]) as res_true:
        accuracy_true = res_true['accuracy']
        log_loss_true = -res_true['log_loss']
        f1_score_true = res_true['f1']

    # ==== Now Print: ==== #
    np.set_printoptions(linewidth=120)
    print('            |     DS (All)    |       ISAR      |      True       |------- T-Test ------')
    print('            |                 |                 |                 |      DS vs ISAR')
    # ---- Accuracy First ---- #
    print('=============================== Accuracy ================================')
    # [A] - Per-Schema Accuracies
    for s in range(len(args.schemas)):
        print(' Schema {:3} | {:.1f}% +/- {:4.2f}% | {:.1f}% +/- {:.2f}% | {:.1f}% +/- {:.2f}% | t={:8.2f}, p={:.2e}'
              .format(args.schemas[s], accuracy_ds[:, s].mean()*100, accuracy_ds[:, s].std()*100,
                      accuracy_isar[:, s].mean()*100, accuracy_isar[:, s].std()*100,
                      accuracy_true[:, s].mean() * 100, accuracy_true[:, s].std() * 100,
    ...
    print(' Global     | {:.1f}% +/- {:.2f}% | {:.1f}% +/- {:.2f}% | {:.1f}% +/- {:.2f}% | t={:8.2f}, p={:.2e}'
          .format(accuracy_ds[:, -1].mean()*100, accuracy_ds[:, -1].std()*100, accuracy_isar[:, -1].mean()*100,
                  accuracy_isar[:, -1].std()*100, accuracy_true[:, -1].mean()*100, accuracy_true[:, -1].std()*100,
                  *ttest_rel(accuracy_ds[:, -1], accuracy_isar[:, -1])))
    print('--------------------------------------------------------------------------')
    ...
    # [A] - Per-Schema PLL
    for s in range(len(args.schemas)):
        print(' Schema {:3} | {:6.2f} +/- {:4.2f} | {:.2f} +/- {:.2f}  | {:.2f} +/- {:.2f}  | t={:8.2f}, p={:.2e}'
              .format(args.schemas[s], log_loss_ds[:, s].mean(), log_loss_ds[:, s].std(),
                      log_loss_isar[:, s].mean(), log_loss_isar[:, s].std(),
                      log_loss_true[:, s].mean(), log_loss_true[:, s].std(),
                      *ttest_rel(log_loss_ds[:, s], log_loss_isar[:, s])))
    # [B] - Now do global
    print('--------------------------------------------------------------------------')
    print(' Global     |  {:.2f} +/- {:.2f} | {:5.2f} +/- {:4.2f}  | {:5.2f} +/- {:4.2f}  | t={:8.2f}, p={:.2e} '
          .format(log_loss_ds[:, -1].mean(), log_loss_ds[:, -1].std(), log_loss_isar[:, -1].mean(),
                  log_loss_isar[:, -1].std(), log_loss_true[:, -1].mean(), log_loss_true[:, -1].std(),
                  *ttest_rel(log_loss_ds[:, -1], log_loss_isar[:, -1])))
    print('--------------------------------------------------------------------------')
    ...
    # [A] - Per-Schema F1
    for s in range(len(args.schemas)):
        print(' Schema {:3} | {:.3f} +/- {:4.3f} | {:.3f} +/- {:.3f} | {:.3f} +/- {:.3f} | t={:8.2f}, p={:.2e}'
              .format(args.schemas[s], f1_score_ds[:, s].mean(), f1_score_ds[:, s].std(), f1_score_isar[:, s].mean(),
                      f1_score_isar[:, s].std(), f1_score_true[:, s].mean(), f1_score_true[:, s].std(),
                      *ttest_rel(f1_score_ds[:, s], f1_score_isar[:, s])))
    # [B] - Now do global
    print('--------------------------------------------------------------------------')
    print(' Global     | {:.3f} +/- {:.3f} | {:.3f} +/- {:.3f} | {:.3f} +/- {:.3f} | t={:8.2f}, p={:.2e}'
          .format(f1_score_ds[:, -1].mean(), f1_score_ds[:, -1].std(), f1_score_isar[:, -1].mean(),
                  f1_score_isar[:, -1].std(), f1_score_true[:, -1].mean(), f1_score_true[:, -1].std(),
                  *ttest_rel(f1_score_ds[:, -1], f1_score_isar[:, -1])))
    print('--------------------------------------------------------------------------')